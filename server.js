...
// GET /recipes/random: Fetch a random recipe for the authenticated user
app.get("/recipes/random", authenticateToken, (req, res) => {
    const userId = req.userId;
  
    db.get(
      `SELECT * FROM recipes WHERE userId = ? ORDER BY RANDOM() LIMIT 1`,
      [userId],
      (err, recipe) => {
        if (err) {
          console.error("Database error:", err);
          return res.status(500).json({ error: "Failed to fetch random recipe" });
        }
        if (!recipe) {
